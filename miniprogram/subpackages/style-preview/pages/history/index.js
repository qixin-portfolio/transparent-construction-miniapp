...
Page({
  ...
  onLoad(options = {}) { this.isMock = mockModeEnabled(options); const customerId = String(options.customerId || ''); this.setData({ customerId, mock: this.isMock }); if (this.isMock) this.load(); else realService.checkAccess().then(() => this.load()).catch(() => this.deny()) },
