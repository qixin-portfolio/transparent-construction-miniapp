...
- 检查结果：Mock service `2/2` 通过；既有日报行为回归 `171/171` 通过；style-preview JS 语法、全量 miniprogram JSON、`git diff --check` 均通过。隔离扫描未发现 `wx.cloud`、`callFunction`、`cloudfunctions`、真实模型或禁用集合名；V2 两处入口仍为 `false`。
